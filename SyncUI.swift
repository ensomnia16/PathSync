...
private let translations: [String: (zh: String, en: String)] = [
    ...
    "newestResolved": ("已采用当前修改日期较新的版本；如有覆盖，旧版本已备份。", "The version with the later current modification date was kept; any replaced versions were backed up."),
    ...
    "notifications": ("同步通知", "Sync notifications"),
    "notificationsOff": ("关闭", "Off"),
    "notificationsIssues": ("冲突或失败时", "On conflicts or failures"),
    "notificationsAll": ("每次同步后", "After every sync"),
    "notificationHint": ("通知默认关闭；选择开启时由 macOS 请求权限。", "Notifications are off by default. macOS asks for permission when you enable them."),
    "notificationDenied": ("未获得通知权限；通知保持关闭。", "Notification permission was not granted; notifications remain off."),
    "notificationIssueTitle": ("路径同步需要注意", "PathSync needs attention"),
    "notificationIssueBody": ("同步遇到冲突或失败，请打开应用查看。", "A sync found a conflict or error. Open the app for details."),
    "notificationDoneTitle": ("路径同步已完成", "PathSync finished"),
    "notificationDoneBody": ("已按设置同步路径。", "Folders were synced using your settings."),
    "menubarOpen": ("打开路径同步", "Open PathSync"),
    "menubarOpenHistory": ("查看同步记录", "View sync history"),
    "menubarQuit": ("退出路径同步", "Quit PathSync"),
    "menubarPending": ("%d 个版本待处理", "%d version(s) need review"),
    "menubarNoPairs": ("尚无已启用路径", "No enabled folders"),
    ...
    "chooseNewest": ("采用当前较新版本", "Use newer current version"),
    "backupRetention": ("备份保留", "Keep backups for"),
    "backupHint": ("删除与编辑一样按上次共同状态判断；覆盖和删除前先备份，到期后在后续同步中清理。", "Deletion and edits use the same last common state. Backups are made before overwrite or deletion and cleaned after expiry on a later sync."),
    ...
    "general": ("通用", "General"),
    "conflictsAndBackups": ("冲突与备份", "Conflicts and backups"),
    "directionSection": ("同步方式", "Sync method"),
    "enabledShort": ("启用", "Enabled"),
    "syncing": ("正在同步…", "Syncing…"),
    "statusReady": ("同步已就绪", "Ready to sync"),
    "lastSync": ("上次同步", "Last sync"),
    "never": ("尚无", "never"),
    "nextRun": ("下次", "Next"),
    "scheduleOff": ("后台同步已关闭", "Scheduled sync is off"),
    "backgroundOn": ("后台同步已开启", "Scheduled sync is on"),
    "pairOK": ("正常", "OK"),
    "pairPending": ("%d 项待处理", "%d pending"),
    "pairLastFailed": ("上次未完成", "Last run incomplete"),
    "pairNotConfigured": ("未选择文件夹", "Folders not set"),
    "folderNotChosen": ("未选择", "Not chosen"),
    "revealInFinder": ("在访达中打开", "Open in Finder"),
    "syncOnceIn": ("按其他方向同步一次", "Sync once in another direction"),
    "removeConfirm": ("移除“%@”？", "Remove “%@”?"),
    "removeConfirmHint": ("只从列表中移除，不会删除任何文件；保存后生效。", "This only removes it from the list. No files are deleted. Save to apply."),
    "unsaved": ("有未保存的更改", "Unsaved changes"),
    "updates": ("软件更新", "Software update"),
    "updateBadge": ("新", "New"),
    "checkForUpdates": ("检查更新…", "Check for Updates…"),
    "checkForUpdatesButton": ("检查更新", "Check now"),
    "autoCheckUpdates": ("自动检查更新", "Check for updates automatically"),
    "autoCheckHint": ("每天最多向 GitHub 查询一次最新发布版本，不发送文件名、路径或其他个人数据。", "Asks GitHub for the latest release at most once a day. No file names, paths, or other personal data are sent."),
    "updateNotChecked": ("尚未检查更新", "Not checked yet"),
    "updateChecking": ("正在检查更新…", "Checking for updates…"),
    "updateUpToDate": ("已是最新版本", "PathSync is up to date"),
