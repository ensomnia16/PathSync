...
struct HistoryChecks {
    static func main() throws {
        ...
        assert(records[2].counts["kept_both"] == 1)
        assert(records[2].events.count == 1)
        assert(records[2].events[0].copyPath == "main (cloud conflict abc).tex")
        assert(!records[2].needsAttention)
        assert(records[3].isPreview)
        assert(records[3].needsAttention)
        assert(records[3].events[0].path == "main.tex")
